 ...
 *
 * Portions Copyrighted 2008 Genuitec, LLC
 *
 * Genuitec, LLC elects to include this software in this distribution
...
TestSupport.prototype = {    
    ...
    init : function (wadlURL) {
    	var wadlFile = wadlURL.match('[^/]+\.wadl$');
    	if (wadlFile) {    		
    		baseURL = wadlURL.substring(0, wadlURL.length - wadlFile[0].length);
    ...
    initFromWadl : function () {
    	this.errorCode = 0;
        this.wdr.getWadlDocument(this.wadlURL);
    },
    
    initFromWadl2 : function(wDoc) {
        if(wDoc != null) {
            ts.wadlDoc = wDoc;
            ...
            if (this.errorCode == 1) {
            	this.updatepage('content', '<span class=bld>Error: Cannot find &lt;resources&gt; element</span><p>Please ensure the above URL points to a valid WADL file.</p>');
            } else {
            	this.updatepage('content', '<span class=bld>Error: Cannot access WADL</span><p>Please make sure the WADL file is accessible at the above URL.</p><p>For locally running applications, please ensure your application is deployed and the server is running.</p>');
            }
        }            
    },
    ...
    getElementsByTagName : function (parent, name, recurseOnce) {
        var results = [];
        if(parent != null && name != null) {
          ...
          for(var i=0;i<childs.length;i++) {
            ...
            if(name == child.nodeName) {
            	...
            	var tResults = this.getElementsByTagName(child, name, false);
            	for (var j = 0; j < tResults.length; j++) {
            		results.push(tResults[j]);
            	}
            }
          }
        }
    ...
    getMethodById : function(id) {
    	var methods = this.getElementsByTagName(ts.wadlDoc.documentElement, "method");
    	for (var i = 0; i < methods.length; i++) {
    		var idAttr = methods[i].attributes.getNamedItem("id");    		
    		if (idAttr && idAttr.nodeValue == id) {
    			return methods[i];
    		}    		
    	}
    	return null;
    ...
    {    
        ...
        if(resource != null && mIndex != -1) {
        	var m;
        	if (!isNaN(parseInt(mIndex))) {
        		m = this.getElementsByTagName(resource, "method")[mIndex];
        	} else {
        		m = this.getMethodById(mIndex);        		
        	}
            request = this.getElementsByTagName(m, "request");
            params = this.getAllParams(resource, m);
        }
        var paramRep = this.getParamRep(params, this.currentMethod);
    ...
    getMethodMimeTypeCombo : function (resource) {
        ...
        for(var j=0;j<methods.length;j++) {
            var m = methods[j];
            var href = m.attributes.getNamedItem("href"); 
            var methodId;
            if (href) {
            	methodId = href.nodeValue.substring(1);
            	m = this.getMethodById(methodId);
            }
            ...
            for(var k=0;k<mimeTypes.length;k++) {
                ...
                var ref = href != null ? methodId : j;
                if(mName == 'GET')
                    str += "  <option class=MnuJmpOpt_sun4 selected value='"+dispName+"["+ref+"]' selected>"+dispName+"</option>";
                else
                    str += "  <option class=MnuJmpOpt_sun4 selected value='"+dispName+"["+ref+"]'>"+dispName+"</option>";
            }
        }   
    ...
    getAllParams : function (resource, method) {
    	var paramList = [];
    	node = resource;
    	while (node && node.tagName != 'resources') {
    		var params = this.getElementsByTagName(node, 'param');
    		for (var i = 0; i < params.length; i++) {
    			paramList.unshift(params[i]);    			
    		}
    		node = node.parentNode;
    	}
    	//reverse to get the elements in order
    	paramList = paramList.reverse();
    	
    	//append params of the "method"
    	var request = this.getElementsByTagName(method, 'request');
    	for (var i = 0; i < request.length; i++) {
    		var params = this.getElementsByTagName(request[i], 'param', true);
			for (var i = 0; i < params.length; i++) {
    ...
    getParamRep : function (params, mName) {
        ...
        if(mName == 'PUT' || mName == 'POST') {   
            str += '<tr><td valign="top"><span id="j_id14"><label for="blobParam" class="LblLev2Txt_sun4">'+
                ...
                '<td><span id="j_id14"><textarea class="TxtAra_sun4 TxtAraVld_sun4" id=blobParam name=params rows=6 cols=65>';
            if (params == null || params.length == 0) str += 'Insert content here.';
            str += '</textarea></span></td></tr>';
        }
        str += '</tbody></table>';
        return str;
    },
    ...
    testResource : function () {
        ...
        if(method == 'POST' || method == 'PUT'){
            ...
            if(blobParam != null && blobParam != undefined && blobParam.length > 0)
                params = blobParam;
        ...
        if(method == 'POST') {
            this.xhr.post(req, mimetype, params, getCallback(this, "testResource2"));
        } else if(method == 'PUT') {
            this.xhr.put(req, mimetype, params, getCallback(this, "testResource2"));
        } else if(method == 'GET') {
        	this.xhr.get(req, mimetype, getCallback(this, "testResource2"));
        } else if(method == 'DELETE') {
            this.xhr.delete_(req, getCallback(this, "testResource2"));
        }
        //if (method != 'GET') 
//        	ts.updateContent(c);
    },
    
    testResource2 : function(c) {
    	ts.updateContent(c);
    },
...
WADLParser.prototype = {
    ...
    getWadlDocument : function (wadlURL) {
		...
		ts.xhr.get(wadlURL, null, getCallback(this, "getWadlDocumentResponse"));
	},
	
	getWadlDocumentResponse : function(wadlData) {
		var rVal = this.getWadlDocument2(wadlData);
		ts.initFromWadl2(rVal);
	},
	
	getWadlDocument2 : function(wadlData) {
        if(wadlData == undefined || wadlData == -1 || wadlData == "" || wadlData.indexOf("RequestFailed -->") != -1) {
        	ts.errorCode = 0;
            ...
            if(app != null) {
              ...
              if(rChilds != null && rChilds.length > 0) {
                ...
                for(var i=0;i<rChilds.length;i++) {
                  ...
                  var wadlDataDoc = ts.xhr.loadXml(wadlData);
                  if (ts.getElementsByTagName(wadlDataDoc.documentElement, 'resources')[0]) {
        			wDocArr.push(wadlDataDoc);        	
        		  }
                }
                if (wDocArr.length == 0) {
                	wDocArr.push(wDoc);
   ...
   hasMethod : function (/*Node*/ n) {
    ...
    getChildren : function (uri) {
    	...
    	ts.xhr.get(baseURL + uri, this.getDefaultMime(), getCallback(this, "getChildrenContent"));
    },
...
XHR.prototype = {
    ...
    httpRequest : function(method, url, mime, content, monitor, callback) {
        var len = 0;
        ...
        var xmlHttpReq = this.connect(method, url, mime, len, callback != null);
        try {
        	if (callback != null) {
	        	xmlHttpReq.onreadystatechange = function() {
	        		try {
	        			if (ts.xhr.isResponseReady(method, xmlHttpReq, content, monitor)) {
		        			var rtext = ts.xhr.getResponse(method, url, xmlHttpReq);
		        			callback(rtext);
	        			}	        		     		
	        		} catch (e) {
            ...
            if (callback == null) {
            	if (this.isResponseReady(method, xmlHttpReq, content, monitor)) {
            		return this.getResponse(method, url, xmlHttpReq);
	            }
            }
        } catch( e ) {
           ts.debug('httpRetrieve(): Caught Exception; name: [' + e.name + '] message: [' + e.message+']');
    ...
    getResponse : function(method, url, xmlHttpReq) {
    	var rtext = xmlHttpReq.responseText;
        if(rtext == undefined || rtext == '' || rtext.indexOf('HTTP Status') != -1) {        	
            var err = method+' RequestFailed RequestFailed -->';
            try {
            	err += ' Status: (' + xmlHttpReq.status + ')';
            } catch (e) {
            	//do nothing
            }
            err += '\n<br/>Response: {<br/>' + xmlHttpReq.responseText + "<br/>}";
    ...
    get : function(url, mime, callback) {
        return this.httpRequest('GET', url, mime, null, true, callback);
    },
    ...
    post : function(url, mime, content, callback) {
        return this.httpRequest('POST', url, mime, content, true, callback);
    },

    put : function(url, mime, content, callback) {
        return this.httpRequest('PUT', url, mime, content, true, callback);
    },

    delete_ : function(url, callback) {
        return this.httpRequest('DELETE', url, 'application/xml', null, true, callback);
    },
...
function getCallback(object, methodName) {
	var args = arguments;
	return function(data) {object[methodName](data, args);};
}
